...
public class TestNewOrderGettersAndSetters {
    ...
    @Test public void testOrderSetters() {
    ...
    @Test(dependsOnMethods = { "testOrderSetters" }) public void testOrderGetters() {
	assertEquals(testOrder.getOrderId(), 1123581321l);
	assertTrue(testOrder.isOpen());
	assertEquals(testOrder.getExpiry().getMillis(), System.currentTimeMillis(), 10_000);
	...
	assertEquals(testOrder.getEpochOpened(), System.currentTimeMillis(), 10_000);
    }

    @Test (enabled = false) public void testOrderClosure() {
