...
public class Stock implements Security {
	...
	private static LinkedHashMap<LocalDate, BigDecimal> fetchHistoricalPrices(String ticker) {
		...
		historicalPriceMap.putAll(fetchHistoricalPrices(ticker));
		if (historicalPriceMap.containsValue(Constants.BIGDECIMAL_MINUS_ONE)) {
			Set<LocalDate> missingPriceEpochs = historicalPriceMap.keySet().stream()
					.filter(localDate -> historicalPriceMap.get(localDate).equals(Constants.BIGDECIMAL_MINUS_ONE))
					.collect(Collectors.toSet());
			fetchMissingPricesFromMarket(ticker, missingPriceEpochs);
			updateStoreMissingPrices(newStock, missingPriceEpochs);
		}
		return historicalPriceMap;
	}
	private static LinkedHashMap<LocalDate, BigDecimal> fetchStoredHistoricalPrices(Stock stock) {
		// Build price map (formerly Stock.populateHistoricalPricesDB -> DBHistoricalPrices.getDBPrices)
		LinkedHashMap<Long, BigDecimal> priceMapFromStore = DATA_STORE.readHistoricalPrices(stock.ticker, EARLIEST_PRICE_DATE);
